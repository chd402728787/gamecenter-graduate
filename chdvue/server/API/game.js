...
exports.add = (req, res) => {        //向info表添加数据
    var sql = 'insert into game (id,gname,img) values (?,?,?)'
    db.query(sql, [req.query.id, req.query.gname, req.query.img], (err, data) => {
        if(err) {
            return res.send('错误：' + err.message)
        }
        if(data.affectedRows > 0) {
...
exports.update = (req, res) => {        //通过id更新数据
    var sql = 'update game set gname = ?, img = ? where id = ?'
    db.query(sql, [req.query.gname, req.query.img, req.query.id], (err, data) => {
        if(err) {
            return res.send('错误：' + err.message)
        }
